...
CSP_META = '<meta http-equiv="Content-Security-Policy" content="default-src \'self\' \'unsafe-inline\' data: blob: https://storage.googleapis.com; img-src \'self\' data: blob: https://storage.googleapis.com https:; style-src \'unsafe-inline\'; script-src \'unsafe-inline\'; connect-src \'none\';">'
...
def generate_dashboard_html(data: Dict[str, Any]) -> str:
    ...
    download_href = gcs_uri if gcs_uri else translated_pdf_b64
    download_attr = f'download="translated_{filename}" target="_blank" rel="noopener noreferrer"'
    download_btn_html = (
        f'<a class="btn" href="{download_href}" {download_attr}>⬇ DOWNLOAD CLEAN DOCUMENT</a>'
        if download_href
        else ""
    )
