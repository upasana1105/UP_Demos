...
def adaptive_translate_tool(
    ...
    try:
        ...
        try:
            ...
            localize_images_in_pdf(output_file_path, target_language_code)
        except Exception as e:
...
def localize_images_in_pdf(pdf_path: str, target_lang: str):
    \"\"\"Extracts page as image, uses Gemini to find and translate image text, 
    and overlays the translated text back on the PDF page.
    \"\"\"
    import fitz  # PyMuPDF
    ...
    for page_num in range(len(doc)):
        ...
        # Render page to image for Gemini to see
        pix = page.get_pixmap()
        img_bytes = pix.tobytes(output="png")
        
        prompt = f\"\"\"
        You are a professional translator. Look at this PDF page.
        Find all text that is embedded in charts, diagrams, or images and is still in English.
        For each label found:
        1. Provide the original text.
        2. Provide the translation into language code '{target_lang}'.
        3. Provide the bounding box [ymin, xmin, ymax, xmax] in normalized coordinates (0 to 1000).
        4. Provide the likely background color of that text area (e.g., 'white', 'blue', or hex).
        
        Return ONLY a JSON array of objects with keys: "text", "translation", "box", "bg_color".
        \"\"\"
        ...
        try:
            ...
            for label in labels:
                box = label.get("box") # [ymin, xmin, ymax, xmax]
                ...
                bg_color = label.get("bg_color", "white").lower()
                color_val = (1, 1, 1) # default white
                if "blue" in bg_color:
                    color_val = (0.1, 0.4, 0.8)
                    
                # Draw filled rectangle to erase old text
                page.draw_rect(fitz.Rect(xmin, ymin, xmax, ymax), color=color_val, fill=color_val)
                
                # Insert translated text
                page.insert_text(fitz.Point(xmin + 2, ymin + 10), label["translation"], fontsize=8, color=(0, 0, 0))
